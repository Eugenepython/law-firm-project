...
export const authMutations = {
  lawFirmLogin: async (_, { username, password }) => {
    ...
    try {
      ...
      const token = jwt.sign({ userId: user.id }, process.env.JWT_SECRET, { expiresIn: '50min' }); // Expires in 20 minutes
      return {
        token,
        user,
      };
  ...
  providerLogin: async (_, { username, password }) => {
    ...
    try {
      ...
      const token = jwt.sign({ userId: user.id }, process.env.JWT_SECRET, { expiresIn: '50min' });
      return {
        token,
        user,
      };
